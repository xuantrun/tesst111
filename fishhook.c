// fishhook.c - Facebook fishhook (BSD License)
...
static vm_prot_t get_protection(void *sectionStart) {
  ...
  kern_return_t info_ret = vm_region_64(task, &address, &size,
      VM_REGION_BASIC_INFO_64, (vm_region_info_64_t)&info, &count, &object);
#else
  mach_msg_type_number_t count = VM_REGION_BASIC_INFO_COUNT;
  vm_region_basic_info_data_t info;
  kern_return_t info_ret = vm_region(task, &address, &size,
      VM_REGION_BASIC_INFO, (vm_region_info_t)&info, &count, &object);
#endif
...
static void perform_rebinding_with_section(struct rebindings_entry *rebindings,
  ...
  if (data_const) mprotect(indirect_symbol_bindings, section->size, oldProtection);
}
